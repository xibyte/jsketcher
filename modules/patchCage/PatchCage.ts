...
export class PatchCage {
  ...
  moveVertex(v: CageVertex, x: number, y: number, z: number): void {
    ...
    // Re-enforce any mirror constraints that involve this vertex
   ...
   */
  mirrorAcrossEdge(patchIdx: number, side: number): number[] {
    const patch = this.patches[patchIdx];
    ...
    // Start from the selected edge's corners and trace through shared vertices.
    const toMirror = this.traceMirrorBoundary(patchIdx, side);

    const result: number[] = [];
    for (const entry of toMirror) {
      result.push(this.mirrorSinglePatch(entry.patchIdx, entry.side, planePoint, planeNormal));
    }

    // Connect shared edges between adjacent mirror patches
    this.stitchMirrorEdges(result);
   ...
   */
  private traceMirrorBoundary(startPatchIdx: number, startSide: number): {patchIdx: number, side: number}[] {
    const result: {patchIdx: number, side: number}[] = [{patchIdx: startPatchIdx, side: startSide}];
    const visited = new Set<number>();
    visited.add(startPatchIdx);

    const startEdge = this.patches[startPatchIdx].getEdgeVertices(startSide);

    // Walk from each corner of the selected edge
    this.walkBoundary(startEdge[0], startPatchIdx, startSide, visited, result);
    this.walkBoundary(startEdge[3], startPatchIdx, startSide, visited, result);
  ...
  private walkBoundary(
    ...
    const adj = this.findAdjacentPatches(fromPatchIdx);
    for (const a of adj) {
      if (visited.has(a.otherIdx)) continue;

      // Does the shared edge touch our boundary corner?
      const fromEdge = this.patches[fromPatchIdx].getEdgeVertices(a.side);
      ...
      const otherPatch = this.patches[a.otherIdx];
      for (let s = 0; s < 4; s++) {
        ...
        if (ev[0] === corner || ev[3] === corner) {
          ...
          this.walkBoundary(nextCorner, a.otherIdx, s, visited, result);
          return;
        }
      }
    }
  ...
  private mirrorSinglePatch(
    ...
    this.patches.push(mp);
    const mirrorIdx = this.patches.length - 1;

    this.mirrorConstraints.push({
   ...
   */
  private stitchMirrorEdges(mirrorIndices: number[]): void {
    if (mirrorIndices.length < 2) return;
    const EPS = 1e-8;

    for (let a = 0; a < mirrorIndices.length; a++) {
      for (let b = a + 1; b < mirrorIndices.length; b++) {
        const pA = this.patches[mirrorIndices[a]];
        const pB = this.patches[mirrorIndices[b]];
        ...
        for (let sA = 0; sA < 4; sA++) {
          ...
          for (let sB = 0; sB < 4; sB++) {
            ...
            for (let i = 0; i < 4; i++) {
              ...
              for (const mc of this.mirrorConstraints) {
   ...
   */
  enforceMirrorConstraints(v: CageVertex): void {
   ...
   */
  enforceAllMirrorConstraints(): void {
    ...
    for (const mc of this.mirrorConstraints) {
      for (const pair of mc.cpPairs) {
        if (!mirrorTargets.has(pair.source)) {
          ...
          this.enforceMirrorConstraints(pair.source);
        }
      }
    }
  }
   ...
   */
  isMirrorTarget(v: CageVertex): boolean {
    for (const mc of this.mirrorConstraints) {
      for (const pair of mc.cpPairs) {
   ...
   */
  removeMirrorConstraint(mc: MirrorConstraint, deletePatch: boolean = true): void {
    const idx = this.mirrorConstraints.indexOf(mc);
    if (idx >= 0) this.mirrorConstraints.splice(idx, 1);
    if (deletePatch) {
      const pi = this.patches.indexOf(this.patches[mc.mirrorPatchIdx]);
      if (pi >= 0) {
        this.patches.splice(pi, 1);
        // Re-index all constraints that reference patches after the deleted one
        for (const m of this.mirrorConstraints) {
          if (m.sourcePatchIdx > pi) m.sourcePatchIdx--;
          if (m.mirrorPatchIdx > pi) m.mirrorPatchIdx--;
        }
        for (const a of this.arcConstraints) {
