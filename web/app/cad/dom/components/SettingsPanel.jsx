...
function applySidebarBg(theme, opacity, blur) {
  const colors = SIDEBAR_BG_COLORS[theme] || SIDEBAR_BG_COLORS.dark;
  const backdropFilter = blur > 0 ? `blur(${blur}px)` : 'none';
  window.dispatchEvent(new CustomEvent('forgecad-sidebar-style', { detail: {
...
export default function SettingsPanel() {
  ...
  useEffect(() => {
    applySidebarBg(theme, 1 - sidebarTransparency / 100, sidebarBlur);
  }, [theme, sidebarTransparency, sidebarBlur]);
  ...
  function changeTheme(t) {
    ...
    applySidebarBg(t, 1 - sidebarTransparency / 100, sidebarBlur);
    ...
    <div className={ls.panel} ref={panelRef} style={{top: topPos, left: leftPos}}>
      ...
      <SubMenu title="Look">
        ...
        <Section title="Toolbar">
          ...
          <div className={ls.sliderRow}>
            <input type="range" min="5" max="15" step="1" value={toolbarTextSize} className={ls.slider}
              onChange={e => {
                const v = Number(e.target.value);
            ...
            />
            <span className={ls.sliderVal}>{toolbarTextSize}px</span>
          </div>
        ...
        <Section title="Sidebar">
          <div className={ls.sliderLabel}>Transparency</div>
          <div className={ls.sliderRow}>
            <input type="range" min="0" max="100" step="1"
              ...
              onChange={e => {
                const v = Number(e.target.value);
                setSidebarTransparencyState(v);
                save('sidebarTransparency', v);
              }}
            />
            <span className={ls.sliderVal}>{sidebarTransparency}%</span>
          </div>
          <div className={ls.sliderLabel}>Blur</div>
          <div className={ls.sliderRow}>
            <input type="range" min="0" max="30" step="1"
              ...
              onChange={e => {
                const v = Number(e.target.value);
                setSidebarBlurState(v);
                save('sidebarBlur', v);
              }}
            />
            <span className={ls.sliderVal}>{sidebarBlur}px</span>
          </div>
        </Section>
      </SubMenu>
      <SubMenu title="Views">
